...
## Generate 32x32 PNG images and labels from the numpy arrays
```python
python get_figs_from_npy.py
```

This command should be invoked from the data_prep directory. Make sure that the `data` directory is a sibling of `data_prep` (i.e., they are both directly within the same parent), and it contains the npy arrays with correct file names.
